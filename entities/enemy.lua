...
function move_enemies()
    ...
    for i, v in ipairs(Enemies) do
        ...
        if Enemies[i].y > 1300 or Enemies[i].y < -50 or Enemies[i].x > 850 or Enemies[i].x < -50 then
...
function add_enemies()
 ...
 function get_new_enemy()
    local rx = math.random(50,1230)
    local ry = math.random(10, 30)
    local rs = math.random(100, 200)
    local rr = math.random(8, 15)
    local enemy = {x=rx, y=ry, health=100, s=rs, r=rr}
    enemy.w = rr*2
    enemy.h = rr*2
